...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/images/nextflow_workflow_subworkflow_module_structure.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/channels_hellonf.png}}
...
\tightlist
\item
  Adicionar \textbf{uma segunda etapa que converte a saudação para
  maiúsculas.}
\item
...
\pandocbounded{\includegraphics[keepaspectratio]{images/workflow_hellonf.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/workflow_translate_hellonf.png}}
...
Nas Partes 1-4 deste curso de treinamento, vimos como usar os blocos de
construção básicos do Nextflow para montar um fluxo de trabalho simples
capaz de processar texto, paralelizar a execução se houvesse múltiplas
entradas e coletar os resultados para processamento adicional.
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{echo} \StringTok{"Hello Containers"} \KeywordTok{|} \ExtensionTok{cowpy} \AttributeTok{{-}c}\NormalTok{ tux}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/cowpy_hellonf.png}}
...
\section{Estrutura de um pipeline
nf-core}\label{estrutura-de-um-pipeline-nf-core}

Todo pipeline nf-core segue o mesmo layout. Aprender esse mapa uma vez
te economiza tempo em qualquer pipeline da coleção.

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{tree} \AttributeTok{{-}L}\NormalTok{ 1 pipelines/nf{-}core/demo}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \textbf{\texttt{conf/modules.config}} --- para descobrir quais
  \texttt{ext.args} já existem e como sobrescrevê-los
\item
  \textbf{\texttt{nextflow\_schema.json}} --- para entender parâmetros
  aceitos (ou use \texttt{-\/-help})
\item
  \textbf{\texttt{assets/schema\_input.json}} --- para saber exatamente
  o que sua samplesheet precisa ter
\end{itemize}
...
\section{\texorpdfstring{Exercício 1 --- Customizar
\texttt{ext.args}}{Exercício 1 --- Customizar ext.args}}\label{exercuxedcio-1-customizar-ext.args}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Enunciado}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

Modifique o pipeline \texttt{nf-core/demo} para que o \texttt{FASTQC}
rode com a flag extra \texttt{-\/-kmers\ 8}, \textbf{sem perder} o
\texttt{-\/-quiet} que já está configurado.

Depois de rodar, prove que a flag entrou inspecionando o
\texttt{.command.sh} do processo FASTQC.

\end{tcolorbox}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, colframe=quarto-callout-tip-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Resposta}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

Crie \texttt{custom.config}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{process }\OperatorTok{\{}
\NormalTok{    withName}\OperatorTok{:} \StringTok{\textquotesingle{}FASTQC\textquotesingle{}} \OperatorTok{\{}
\NormalTok{        ext}\OperatorTok{.}\NormalTok{args }\OperatorTok{=} \StringTok{\textquotesingle{}{-}{-}quiet {-}{-}kmers 8\textquotesingle{}}
    \OperatorTok{\}}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{nextflow}\NormalTok{ run nf{-}core/demo }\AttributeTok{{-}profile}\NormalTok{ docker,test }\AttributeTok{{-}{-}outdir}\NormalTok{ demo{-}ex1 }\AttributeTok{{-}c}\NormalTok{ custom.config}
\end{Highlighting}
\end{Shaded}

Encontre o hash do FASTQC na saída (ex: \texttt{{[}ca/5b0f3e{]}}) e
inspecione:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cat}\NormalTok{ work/ca/5b0f3e}\PreprocessorTok{*}\NormalTok{/.command.sh}
\end{Highlighting}
\end{Shaded}

A linha do \texttt{fastqc} deve conter \texttt{-\/-quiet\ -\/-kmers\ 8}.

\textbf{Ponto-chave}: se você tivesse escrito só
\texttt{ext.args\ =\ \textquotesingle{}-\/-kmers\ 8\textquotesingle{}},
o \texttt{-\/-quiet} teria sumido --- \texttt{ext.args} substitui, não
soma.
...
\section{Exercício 2 --- Debugando uma
samplesheet}\label{exercuxedcio-2-debugando-uma-samplesheet}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Enunciado}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

Crie uma samplesheet \texttt{bad\_sheet.csv} que quebre a validação de
\textbf{três formas diferentes ao mesmo tempo}:
...
Rode o pipeline e observe que os três erros aparecem numa mesma
execução.
...
\texttt{bad\_sheet.csv}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sample,fastq\_2}
\NormalTok{SAMPLE UM,/tmp/reads.txt}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Falta a coluna \texttt{fastq\_1} (obrigatória ---
  \texttt{required:\ {[}"sample",\ "fastq\_1"{]}})
\item
  \texttt{.txt} não bate com o padrão
  \texttt{.f(ast)?q\textbackslash{}.gz\$}
\item
  \texttt{SAMPLE\ UM} tem espaço --- falha no padrão
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{nextflow}\NormalTok{ run nf{-}core/demo }\AttributeTok{{-}profile}\NormalTok{ docker,test }\AttributeTok{{-}{-}outdir}\NormalTok{ demo{-}ex2 }\AttributeTok{{-}{-}input}\NormalTok{ bad\_sheet.csv}
\end{Highlighting}
\end{Shaded}

O nf-schema reporta os três erros de uma vez --- você corrige tudo numa
iteração só, sem ter que rodar três vezes.
...
\section{Exercício 3 --- Adicionando um módulo
nf-core}\label{exercuxedcio-3-adicionando-um-muxf3dulo-nf-core}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Enunciado}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

Suponha que você precisa adicionar \texttt{samtools/faidx} a um pipeline
hipotético para indexar um FASTA.
...
\tightlist
\item
  Como você descobre se o módulo existe e o que ele espera de entrada?
\item
  Qual comando instala o módulo no diretório do pipeline?
\item
  Escreva a linha \texttt{include} para importá-lo no workflow.
\item
  Se você quisesse passar a flag extra \texttt{-\/-fai-idx-name} ao
  \texttt{samtools\ faidx}, qual seria o bloco \texttt{custom.config}?
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{nf{-}core}\NormalTok{ modules info samtools/faidx}
\end{Highlighting}
\end{Shaded}

ou pelo site:
\href{https://nf-co.re/modules/samtools/faidx}{nf-co.re/modules/samtools/faidx}.
Ambos mostram inputs/outputs esperados.

\textbf{b)} Dentro da raiz do pipeline:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{nf{-}core}\NormalTok{ modules install samtools/faidx}
\end{Highlighting}
\end{Shaded}

Isso cria \texttt{modules/nf-core/samtools/faidx/} e registra em
\texttt{modules.json}.

\textbf{c)} No arquivo de workflow:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{include }\OperatorTok{\{}\NormalTok{ SAMTOOLS\_FAIDX }\OperatorTok{\}}\NormalTok{ from }\StringTok{\textquotesingle{}../modules/nf{-}core/samtools/faidx/main\textquotesingle{}}
\end{Highlighting}
\end{Shaded}

\textbf{d)} No \texttt{custom.config}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{process }\OperatorTok{\{}
\NormalTok{    withName}\OperatorTok{:} \StringTok{\textquotesingle{}SAMTOOLS\_FAIDX\textquotesingle{}} \OperatorTok{\{}
\NormalTok{        ext}\OperatorTok{.}\NormalTok{args }\OperatorTok{=} \StringTok{\textquotesingle{}{-}{-}fai{-}idx{-}name custom.fai\textquotesingle{}}
    \OperatorTok{\}}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
...
\chapter{Próximos passos}\label{pruxf3ximos-passos}

Você viu tudo com o \texttt{nf-core/demo}, mas o padrão é o mesmo para
qualquer pipeline da coleção. O que muda entre eles é o número de
parâmetros, o volume de dados e o tempo de execução --- mas o mecanismo
de customização (\texttt{-profile}, \texttt{-params-file}, \texttt{-c}
com \texttt{ext.args}) é idêntico.
...
\tightlist
\item
  \textbf{\href{https://nf-co.re/rnaseq}{nf-core/rnaseq}} ---
  quantificação de expressão gênica a partir de RNA-seq
\item
  \textbf{\href{https://nf-co.re/sarek}{nf-core/sarek}} --- chamada de
  variantes germinativas e somáticas (WGS/WES)
\item
  \textbf{\href{https://nf-co.re/ampliseq}{nf-core/ampliseq}} ---
  análise de amplicons (16S, ITS)
\item
  \textbf{\href{https://nf-co.re/atacseq}{nf-core/atacseq}} --- ATAC-seq
\item
  \textbf{\href{https://nf-co.re/mag}{nf-core/mag}} --- metagenômica
  assembly
\end{itemize}
...
\tightlist
\item
  Ler a aba \textbf{Introduction} e o \textbf{subway map} no site
\item
  Ler a aba \textbf{Parameters} e escolher os obrigatórios
\item
  Rodar o \texttt{-profile\ test} para confirmar que o ambiente funciona
\item
  Preparar a samplesheet no formato do
  \texttt{assets/schema\_input.json}
\item
  Rodar com seus dados reais + \texttt{custom.config} para ajustar
  recursos/\texttt{ext.args}
\end{enumerate}

Comunidade: \href{https://nf-co.re/join/slack}{nf-core Slack} para tirar
dúvidas --- muito ativa e receptiva.
